...
with open(jobstreet_params.full_id_path_cs, "r", encoding="utf-8") as data:
...
print(f"{jobstreet_params.job_portal} {jobstreet_params.job_category} {jobstreet_params.keyword_name}:")

# Buat kombinasi job_title & company
existing_jobs = set()
for job in full_id:
    existed = f"{job['job_title']} - {job['company']}"
...
for job in new_id:
    new = f"{job['job_title']} - {job['company']}"
...
# # Simpan ke list full
# print(f"Total ID {jobstreet_params.ISorCS_Jobs} sebelumnya: {len(full_id)}")

# full_id += new_scraped
# with open(jobstreet_params.full_id_path_cs,
#     "w",
#     encoding="utf-8",
# ) as f:
#     json.dump(full_id, f, ensure_ascii=False, indent=4)

# print(f"Total ID {jobstreet_params.ISorCS_Jobs}: {len(full_id)}")